...
nslcd nslcd/ldap-uris string ldap://ldap-server-a
...
ldap-auth-config ldap-auth-config/ldapns/ldap-server string ldap://ldap-server-a/
...
sed -i 's,uri ldapi:///,uri ldap://ldap-server-a,g' /etc/ldap.conf #can use something different from "/" as seperator
...
echo "nsf-server-a:/var/nfsshare/testing  /mnt/test nfs defaults 0 0" >> /etc/fstab
